...
def fig3():
    ...
    for ax, (name, exp) in zip(axes, [
        ...
        ax.bar(range(len(au_s)), au_s, color=BLUE, edgecolor="black", linewidth=0.8)
...
def fig4():
    def load_baselines(ds):
        ...
        with (base / "simple_baseline_results.json").open() as f:
            simple = json.load(f)
        nn = simple["nn_on_ib"].get("metrics", simple["nn_on_ib"])
        raw = simple["raw_ot"].get("metrics", simple["raw_ot"])
        return scot, nn, raw
    ...
    metrics = [("FOSCTTM$\\downarrow$", "foscttm_mean", "foscttm"),
               ("LT RNA$\\to$ATAC", "lt_rna_to_atac", "lt_rna_to_atac"),
               ("LT ATAC$\\to$RNA", "lt_atac_to_rna", "lt_atac_to_rna"),
               ("ARI", "joint_ari", "joint_ari")]
    ...
    for row_i, (ds_name, mosaic, scot, nn, raw) in enumerate([
        ...
        for col_i, (title, key_aggr, key_base) in enumerate(metrics):
            ax = axes[row_i, col_i]
            def _flt(v):
                ...
                try:
                    ...
                    return float('nan')
            vals = [
                _flt(mosaic.get(f"{key_aggr}_mean", np.nan)),
                _flt(nn.get(key_base, np.nan)),
                _flt(scot.get(key_base, np.nan)),
                _flt(raw.get(key_base, np.nan)),
            ]
            errs = [_flt(mosaic.get(f"{key_aggr}_std", 0)), 0.0, 0.0, 0.0]
            labels = ["MOSAIC", "NN-on-IB", "SCOT", "Raw"]
            colors = [BLUE, GREEN, "#999999", "#cccccc"]
            ax.bar(range(4), vals, yerr=errs, color=colors,
                   edgecolor="black", linewidth=0.8, capsize=6)
            ax.set_xticks(range(4))
            ax.set_xticklabels(labels, rotation=0)
            ax.set_title(f"{ds_name} — {title}" if row_i == 0 else title)
            if col_i == 0:
...
def fig7():
    ...
    fig, axes = plt.subplots(1, len(configs), figsize=(22, 6))
    for ax, (name, exp) in zip(axes, configs):
        ...
        ax.plot([0, 1], [0, 1], color="gray", linestyle="--", linewidth=1.5, label="perfect")
        ax.plot(x, y, marker="o", color=BLUE, linewidth=2.5, markersize=10, label="observed")
